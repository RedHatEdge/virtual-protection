...
| **Pacemaker `VirtualDomain` (not OpenStack / OpenShift Virt)** | A 3-node substation cluster doesn't need a multi-tenant scheduler. Pacemaker's `VirtualDomain` is the simplest tool that solves the failover problem, with a 20-year operational track record in utility / telco. |
